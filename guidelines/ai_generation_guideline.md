...
- 例外として、作者へのDM申請後にボイスアセットや合成音声モデル等が個別提供された場合は、提供されたアセットのみ、提供時の個別通知と[ボイスアセット利用ガイドライン](../voice/VOICE_ASSET_GUIDELINE.md)の範囲で利用できます。本リポジトリの `/voice/` フォルダ内にある公開ファイルは、この例外の対象ではありません。
